...
func (rf *Raft) persist() {
	...
	w := new(bytes.Buffer)
	e := gob.NewEncoder(w)
	// Persisting the 4 base variables. Other variables can be derived from these.
...
func (rf *Raft) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) {
	...
	if rf.lastLog-1 >= 0 {
		...
		// Extra layer of security to make sure no commited value is deleted
		// It is much easier for the candidate to send its last Commit Index, instead of the above code and risking overwriting
...
func (rf *Raft) leaderRoutine() {
	...
	if count >= threshold {
		...
		// fmt.Printf("%d Leader's lastlogindex = %d \n", rf.me, lastLogIndex)
