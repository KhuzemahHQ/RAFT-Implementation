...
type RaftKV struct {
	...
	putResult chan string
}

func (kv *RaftKV) Get(args *GetArgs, reply *GetReply) {
	// Your code here.

	// fmt.Printf("Get RPC called for %d with key, seq=%s, %d \n", kv.me, args.Key, args.Seq)
...
func (kv *RaftKV) PutAppend(args *PutAppendArgs, reply *PutAppendReply) {
	...
	// fmt.Printf("PutAppend RPC called for %d , details: key,value,op,seq = %s, %s, %s, %d\n", kv.me, args.Key, args.Value,args.Op, args.Seq)

	kv.mu.Lock()
	if _, ok := kv.reqs[args.Seq]; ok {
		// fmt.Printf("%d with seq%d already had value %t \n",kv.me, args.Seq, val)
		kv.mu.Unlock()
		return
	} else {
		// kv.reqs[args.Seq] = true
	}
	kv.mu.Unlock()
	...
	if isLeader {
		// fmt.Printf("term, leader, me = %d, %t  %d \n", term, isLeader, kv.me)
		// fmt.Printf("Leader %d got PutAppend RPC called details: key,value,op,seq = %s, %s, %s, %d\n", kv.me, args.Key, args.Value,args.Op, args.Seq)
		reply.WrongLeader = false
		// fmt.Printf("%t  \n", reply.WrongLeader)
		return
	} else {
...
func StartKVServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int) *RaftKV {
	...
	kv.putResult = make(chan string)
	kv.reqs = make(map[int64]bool)
